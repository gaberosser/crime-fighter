...
def network_linkages(data_source, max_t, max_d, data_target=None, chunksize=2**18, remove_coincident_pairs=False):
    ...
    The sign convention is (target - source).  Distances are defined on the network.
    ...
    if data_target is not None:
        if data_target.graph is not data_source.target:
            raise AttributeError("Network points are defined on different graphs.")
