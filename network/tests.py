...
if __name__ == "__main__":
    ...
    x_pts = np.random.rand(100) * (xmax - xmin) + xmin
    y_pts = np.random.rand(100) * (ymax - ymin) + ymin

    grid_edge_index = itn_net.build_grid_edge_index(xmin, xmax, ymin, ymax, 50)
    net_points = []
    snap_dists = []
    for x, y in zip(x_pts, y_pts):
    ...
    st_net_point_array = models.NetworkSpaceTimeData(
        zip(sorted(np.random.rand(100)), net_points)
    )