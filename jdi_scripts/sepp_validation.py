...
OUT_SUBDIR = 'sepp_validation_2013-2014'

# global parameters
num_sample_points = 30
grid_size = 250  # metres
niter = 100  # number of SEPP iterations before convergence is assumed
num_validation = 365  # number of predict - assess cycles
start_date = datetime.datetime(2013, 1, 1)  # first date for which data are required
...
def train_sepp(crime_type='burglary', borough=None):
    from jdi.data import cris
    
    start_dt = datetime.datetime(2014, 1, 1)
    end_dt = datetime.datetime(2015, 1, 1)
    data, t0, cid = cris.get_cris_data(crime_type=crime_type, borough=borough, start_dt=start_dt, end_dt=end_dt)
    sepp = pp_models.SeppStochasticNn(data=data, **model_kwargs)
...
if __name__ == '__main__':
    from jdi.data import boundary, cris
    ...
    data, t0, cid = cris.get_cris_data(start_dt=start_date, end_dt=end_date, crime_type=crime_type, borough=borough)    
